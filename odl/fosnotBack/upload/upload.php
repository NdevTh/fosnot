...
// insérer des photos aux nouveaux évènements
if (isset($_FILES['newevent'])) {
    $tmpName = $_FILES['newevent']['tmp_name'];
    $name = $_FILES['newevent']['name'];
    $size = $_FILES['newevent']['size'];
    $error = $_FILES['newevent']['error'];
    ...
    if (in_array($extension, $extensions) && $size <= $maxSize && $error == 0) {
        ...
        move_uploaded_file($tmpName, '../fosnotFront/image/' . $name);
        // on insère la requete
        $req = $db->prepare("INSERT INTO `events` (eve_type,eve_directory) VALUES ('futur',$name)");
        $req->execute([$name]);
        echo "Image enregistrée";
    } else {
        echo "Une erreur est survenue";
